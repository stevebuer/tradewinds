...
class VolDashboard:
    ...
    def load_vol_metrics(self, symbol: str) -> pd.DataFrame:
        query = """
    ...
    def plot_atm_iv_time_series(self, df: pd.DataFrame, symbol: str):
        atm = (
        ...
        plt.plot(atm["timestamp"], atm["atm_iv"], label="ATM IV")
    ...
    def plot_skew_time_series(self, df: pd.DataFrame, symbol: str):
        skew = (
            df.sort_values("expiration")
              .groupby("timestamp")
              .first()
        ...
        plt.figure(figsize=(10, 4))
        plt.plot(skew["timestamp"], skew["skew"], label="Skew", color="orange")
